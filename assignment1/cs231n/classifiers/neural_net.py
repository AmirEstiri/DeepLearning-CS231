...
class TwoLayerNet(object):
  ...
  """
    ...
    """
        ...
        loss += np.sum(np.log(normalizer)) / N
        ...
        normalized[np.arange(N), y] -= 1
        normalized /= N
        dL2 = normalized
        dW2 = np.dot(RL1.T, dL2)
        db2 = np.sum(dL2, axis=0)
        dRL1 = np.dot(dL2, W2.T)
        dL1 = np.dot(dL2, W2.T)
        dL1[L1 < 0] = 0
        dW1 = np.dot(X.T, dL1)
    ...
    """
        ...
        L2 = RL1.dot(W2) + b2
        test_scores = L2
        index_matrix = np.tile(np.arange(L2.shape[1]), (L2.shape[0], 1))
        y_pred = index_matrix[np.arange(len(test_scores)), np.argmax(test_scores, axis=1)]
        ###########################################################################
        #                              END OF YOUR CODE                           #
        ###########################################################################
